...
class App(models.Model):
    ...
    tier = models.ForeignKey(
        "access_app.RequestAccessTier", on_delete=models.CASCADE, null=True, blank=True
    )
    schema_title = models.CharField(
        max_length=100, blank=True, help_text="Title of the API Schema."
    )
    schema_name = models.CharField(
        max_length=100, blank=True, help_text="Description for API Schema."
    )
    ...
    def get_schema_url(self):
        ...
        return join(
            settings.HOST, reverse("schema", kwargs={"app_id": self.app_id})[1:]
        )

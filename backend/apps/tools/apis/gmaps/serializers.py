...
class LocationSearchInputSerializer(serializers.Serializer):
    ...
    lat = serializers.FloatField(
        required=False, help_text="Latitude (alternative to location string)"
    )
    lng = serializers.FloatField(
        required=False, help_text="Longitude (alternative to location string)"
    )
    ...
    type = serializers.CharField(
        required=False, help_text="Place type (e.g., restaurant, gas_station, hospital)"
    )
    ...
    name = serializers.CharField(
        required=False, help_text="Specific place name to search for"
    )
    ...
    open_now = serializers.BooleanField(
        required=False, help_text="Filter places that are open now"
    )
...
class GeocodingInputSerializer(serializers.Serializer):
    ...
    address = serializers.CharField(
        required=False, help_text="Address to geocode (for forward geocoding)"
    )
    lat = serializers.FloatField(
        required=False, help_text="Latitude (for reverse geocoding)"
    )
    lng = serializers.FloatField(
        required=False, help_text="Longitude (for reverse geocoding)"
    )
    ...
    region = serializers.CharField(
        required=False, help_text="Region code to bias results"
    )
    ...
    location_type = serializers.ListField(
        ...
        help_text=(
            ...
            "(e.g., rooftop, range_interpolated, geometric_center",
            "approximate)",
        ),
    )
...
class DistanceDirectionsInputSerializer(serializers.Serializer):
    ...
    origins = serializers.CharField(help_text="Origin address or coordinate")
    destinations = serializers.CharField(help_text="Destination address or coordinate")
    mode = serializers.ChoiceField(
        choices=TRAVEL_MODES, default="driving", help_text="Travel mode"
    )
    units = serializers.ChoiceField(
        choices=UNITS, default="metric", help_text="Unit system"
    )
    ...
    optimize_waypoints = serializers.BooleanField(
        default=False, help_text="Optimize waypoint order"
    )
    alternatives = serializers.BooleanField(
        default=False, help_text="Return alternative routes"
    )
    departure_time = serializers.IntegerField(
        required=False, help_text="Departure time as Unix timestamp"
    )
    arrival_time = serializers.IntegerField(
        required=False, help_text="Arrival time as Unix timestamp"
    )

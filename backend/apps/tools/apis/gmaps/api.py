...
GMAP_RETRY = Retry(
    total=3, backoff_factor=0.5, status_forcelist=googlemaps.client._RETRIABLE_STATUSES
)
...
class GMapViewSet(ViewSet):
    ...
    def generate_google_map_link(self, embed_type: EmbedUrlType, **kwargs):
        if embed_type == EmbedUrlType.place:
            return (
                f"https://www.google.com/maps/search/?api=1&query={kwargs.get('query')}"
            )
    ...
    def location(self, request):
        ...
        self.raise_on_invalid_map_status(
            api_status, f"Error searching for nearby places ({api_status})"
        )
    ...
    def geocoding(self, request):
        ...
        response_data = {"type": action_type, "results": results}
    ...
    def distance_and_directions(self, request):
        ...
        if directions_result:
            ...
            try:
                ...
                if isinstance(e, GMapUnexpectedError):
                    logger.error(
                        f"Distance matrix failed unexpectedly ({distance_matrix['status']})"
                    )
        ...
        output_serializer = DistanceDirectionsOutputSerializer(

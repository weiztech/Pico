...
from lit_reviews.database_imports.embase import *




### added database_imports modules
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for filename in os.listdir('./manual_imports/{0}/embase'.format(lit_review_id)):
        ...
        #return True
                parse_text('./manual_imports/{0}/embase/{1}'.format(lit_review_id, filename) , search_terms, lit_review_id, years_back)

            elif filename.lower().find('exclude') != -1 and filename.find(".csv") != -1:

                # exclude file.  parse this differently.
                add_excluded_searches('./manual_imports/{0}/embase/{1}'.format(lit_review_id, filename), lit_review_id, years_back)

...
from lit_reviews.database_imports.cochrane import *


### moved to lit_reviews.dabase_imports.cochrane ##
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for filename in os.listdir('./manual_imports/{0}/cochrane'.format(lit_review_id)):
            ...
            if filename.find(".txt") != -1 and filename.lower().find('exclude') == -1:
                ...
                search_terms = filename.replace("_STAR", "*")
                print("search terms: " + str(search_terms))

        #return True
                parse_text('./manual_imports/{0}/cochrane/{1}'.format(lit_review_id, filename) , search_terms, lit_review_id, years_back)

            elif filename.lower().find('exclude') != -1 and filename.find(".csv") != -1:

                # exclude file.  parse this differently.
                add_excluded_searches('./manual_imports/{0}/cochrane/{1}'.format(lit_review_id, filename), lit_review_id, years_back)
